...
local function sh_stream_in_split(cmd, opts)
  ...
  -- keep it like a scratch buffer, but we will render via a terminal emulator
  ...
  vim.api.nvim_buf_set_lines(buf, 0, -1, false, {}) -- terminal will own the display

  -- Open a terminal *renderer* for this buffer. It interprets ANSI escapes.
  local term_chan = vim.api.nvim_open_term(buf, {})

  -- small helper to check if the output window is scrolled to bottom
  ...
  -- batching writes is still nice for performance
  local pending = ""
  local function add_lines(lines, prefix)
    ...
    if #t > 0 then
      -- terminals expect CRLF for proper line movement
  ...
  local function flush()
    ...
    -- Send raw bytes to the terminal emulator; it will render colors.
  ...
  -- Show the echoed command at the top
  add_lines({ "$ " .. cmd, "" })
  ...
  -- Feed stdin (if provided) and close it
